...
- **스케줄링 (Scheduling)**: 설정된 시간에 특정 작업을 자동으로 실행하기 위해 `node-cron`과 같은 라이브러리 또는 AWS EventBridge와 같은 클라우드 서비스를 활용한다.
- **결제 연동**: 예치금 및 벌금 시스템을 위해 **Toss Payments, I'mport** 등 외부 PG(결제 대행)사와의 연동이 필요하다.
- **이벤트 기반 아키텍처**: '출석 체크', '벌금 부과' 등 각 이벤트는 Next.js API Routes에서 발생하고, 그 결과는 Redis Pub/Sub을 통해 시그널링 서버에 전달되어 클라이언트에게 실시간으로 알림을 보낸다.
