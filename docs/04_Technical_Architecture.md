...
graph TD
    ...
    subgraph Next.js Fullstack Tier (Vercel)
        B(Next.js Server - Frontend & API Routes)
    ...
    A -- HTTPS --> B
    B -- Server-Side Rendering & API Handling --> A
...
### 2.1. Next.js 풀스택 (Next.js Fullstack Tier)

- **기술**: **Next.js (React) with API Routes**
- **역할**: 사용자에게 보여지는 모든 UI와 상호작용을 담당하며, Next.js API Routes를 통해 CoUp 서비스의 **핵심 비즈니스 로직**을 처리합니다. 데이터 영속성, 사용자 인증/인가, 복잡한 도메인 규칙 등을 담당하며, 시그널링 서버에 RESTful API를 제공합니다.
...
- **동작 방식**:
  ...
  2.  수신한 메시지를 DB에 저장하거나 관련 비즈니스 로직을 처리하기 위해 **Next.js API Routes의 내부 API를 호출**합니다.
  3.  Next.js API Routes에서 특정 이벤트(예: 새로운 공지 등록)가 발생하면, **Redis Pub/Sub**을 통해 해당 이벤트를 구독(Subscribe)하여 클라이언트에게 실시간으로 푸시합니다.
...
- **캐시 & 메시지 브로커 (Redis on AWS ElastiCache)**:
  - **캐시**: Next.js API Routes의 DB 부하를 줄이기 위한 캐싱 용도로 사용됩니다.
  - **메시지 브로커 (Pub/Sub)**: Next.js API Routes와 시그널링 서버 간의 **느슨한 결합(Loose Coupling)**을 위해 사용됩니다. Next.js API Routes가 특정 채널에 이벤트를 발행(Publish)하면, 시그널링 서버가 이를 구독하여 처리하는 방식으로 서버 간 직접적인 의존성을 제거합니다.